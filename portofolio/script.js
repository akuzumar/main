...
const typingWrap = typingText ? typingText.closest(".typing-wrap") : null;
...
function lockTypingWrapHeight() {
  if (!typingWrap) return;

  const fullText = safeText(getDictionary().heroTagline);
  if (!fullText) return;

  const wrapWidth = typingWrap.getBoundingClientRect().width;
  if (!wrapWidth) return;

  const computedStyle = window.getComputedStyle(typingWrap);
  ...
  measureNode.style.width = `${wrapWidth}px`;
  ...
  if (measuredHeight > 0) {
    typingWrap.style.minHeight = `${measuredHeight}px`;
  }
}

function queueLockTypingWrapHeight() {
  if (typingHeightResizeRaf) {
    window.cancelAnimationFrame(typingHeightResizeRaf);
  }

  typingHeightResizeRaf = window.requestAnimationFrame(() => {
    typingHeightResizeRaf = 0;
    lockTypingWrapHeight();
  });
}
...
function initTypingWrapHeightLock() {
  if (!typingWrap) return;

  window.addEventListener("resize", queueLockTypingWrapHeight);
  window.addEventListener("orientationchange", queueLockTypingWrapHeight);

  if (document.fonts && "ready" in document.fonts) {
    document.fonts.ready.then(() => {
      queueLockTypingWrapHeight();
    }).catch(() => {
...
function normalizeConfiguredGalleryItem(entry, index, dictionary) {
  ...
  const title = safeText(normalizedEntry.title, caption); // Gunakan caption sebagai fallback title
  const description = safeText(normalizedEntry.description, title); // Gunakan title sebagai fallback description
...
function createGalleryTile(source, dictionary) {
  ...
  // Simpan title dan description sebagai data attribute
  if (source.title) {
    image.dataset.galleryTitle = source.title;
  }
  if (source.description) {
    image.dataset.galleryDescription = source.description;
  }
...
function createGalleryLightboxButton(text, ariaLabel, isCloseButton = false) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "gallery-lightbox-btn";
  if (isCloseButton) {
    button.classList.add("gallery-lightbox-close");
  }
...
function updateGalleryLightboxLabels(dictionary) {
  ...
  closeButton.textContent = safeText(dictionary.modalClose, "Tutup");
}
...
function ensureGalleryLightbox() {
  ...
  const close = createGalleryLightboxButton(
    safeText(getDictionary().modalClose, "Tutup"),
...
function openGalleryLightbox(source, altText, titleText, descriptionText = "", captionText = "") {
  ...
  // Gunakan title sebagai judul utama, fallback ke caption atau alt
  const resolvedTitle = buildMediaTitle(titleText, captionText || altText, dictionary);
  // Gunakan description sebagai deskripsi, fallback ke title atau caption
  const resolvedDescription = buildMediaDescription(descriptionText, titleText || captionText || altText, dictionary);
...
function initGalleryLightbox() {
  ...
  galleryWall.addEventListener("click", (event) => {
    ...
    // Dapatkan data dari gambar (sekarang gambar menyimpan data)
    const titleFromData = image.dataset.galleryTitle || "";
    const descriptionFromData = image.dataset.galleryDescription || "";
  ...
  galleryWall.addEventListener("keydown", (event) => {
    ...
    const titleFromData = target.dataset.galleryTitle || "";
    const descriptionFromData = target.dataset.galleryDescription || "";
  ...
  detailFigureImages.forEach((image) => {
    ...
    // Untuk gambar di halaman detail, kita bisa set data dari atribut yang ada
    // atau menggunakan nilai default
    if (!image.dataset.galleryTitle) {
      const figure = image.closest(".detail-figure");
      const captionElement = figure ? figure.querySelector("figcaption") : null;
      const captionText = captionElement ? safeText(captionElement.textContent) : "";
      image.dataset.galleryTitle = captionText;
      image.dataset.galleryDescription = captionText;
    }
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    const titleFromData = image.dataset.galleryTitle || image.alt || captionText;
    const descriptionFromData = image.dataset.galleryDescription || captionText;
  ...
  document.addEventListener("keydown", (event) => {
    ...
    const titleFromData = target.dataset.galleryTitle || target.alt || captionText;
    const descriptionFromData = target.dataset.galleryDescription || captionText;
